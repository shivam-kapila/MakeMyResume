...
router.get('/user/data',passport.authenticate('jwt',{session:false}),(req,res,next)=>{
...
router.post('/user/data',passport.authenticate('jwt',{session:false}),(req,res,next)=>{
  const token=req.query.token
  ...
  if(token){
    ...
    User.findOne({username:req.user.username},function(err,user){
      ...
      res.json({msg:"true data!",user:user})
    })
  }
  else
    res.status(401).json({msg:"Unauthorized!!No Token",err:"Token not found"})
